...
def generate_and_save_data(generator, SE, save_dir, rounds, mode, save_frequency, complexity, n_samples, problem_type, df_subset=None,
    ...
    for r in range(rounds+1):
        ...
        # Log seed synchronization
        log_seed(f"[Round {r}] GPU {local_rank} SYNCHRONIZED SEED: {deterministic_seed}")

        # Ensure all GPUs have set their seeds before proceeding
        if torch.distributed.is_initialized():
            torch.distributed.barrier()
            log_seed(f"[Round {r}] GPU {local_rank} passed seed sync barrier")
        ...
        # === CRITICAL: Ensure all GPUs completed dialog generation before proceeding ===
        if torch.distributed.is_initialized():
            torch.distributed.barrier()
            log_seed(f"[Round {r}] GPU {local_rank} passed dialog generation barrier")
