...
if generate_data:
    print("[Data Generation] Starting training data generation")
    ...
    print("[Data Generation] Training data completed")

    print("[Data Generation] Starting validation data generation")
    generate_and_save_data(generator=generator, SE=SE, save_dir=save_dir, 
                           ...
                           tokens_to_keep=tokens_to_keep, calculate_end_index=calculate_end_index, verbose=True)
    print("[Data Generation] Validation data completed")

    print("[Data Generation] Starting testing data generation")
    generate_and_save_data(generator=generator, SE=SE, save_dir=save_dir, 
                           ...
                           tokens_to_keep=tokens_to_keep, calculate_end_index=calculate_end_index, verbose=True)
    print("[Data Generation] Testing data completed")

if not generate_data:
    print("[Training] Loading training data loaders")
    ...
    print("[Training] Training data loaders created")

    print("[Training] Loading validation data loaders")
    validation_encoder_data_loaders = generate_data_loaders(mode='val', save_dir=save_dir, data_rounds=val_data_rounds, n_samples=n_samples, df_subset=df_val,
                                                            ...
                                                            tokens_to_keep=tokens_to_keep, batch_size=encoder_decoder_batch_size, gpu_seed=gpu_seed, verbose=True)
    print("[Training] Validation data loaders created")

    print("[Training] Loading testing data loaders")
    testing_encoder_data_loaders = generate_data_loaders(mode='test', save_dir=save_dir, data_rounds=test_data_rounds, n_samples=n_samples, df_subset=df_test,
                                                         ...
                                                         tokens_to_keep=tokens_to_keep, batch_size=encoder_decoder_batch_size, gpu_seed=gpu_seed, verbose=True)
    print("[Training] Testing data loaders created")
    ...
    print("[Training] Trainable parameters per layer:", count_trainable_parameters(layer_encoder))
    ...
    print("[Encoder Training] Starting encoder training")
    for i in range(training_epochs):
        if i in learning_rate_reduction_factors.keys():
            for n in range(len(layer_numbers)):
                for param_group in optimizers[n].param_groups:
                    ...
                    print(f"[Encoder Training] Learning Rate changed to: {param_group['lr']}")
        ...
        for n, n_layer in enumerate(layer_numbers):
            ...
            if not i % train_freq_print:
                print(f"[Encoder Training] Layer {n_layer}: validation avg correct digits: {average_correct_digits:.4f}, accuracy per digit: {error_per_digit}")

            encoders[n].train()

        if not i % train_freq_print:
            print(f"[Encoder Training] Epoch {i}: Train Loss={running_losses.mean():.4f}, Val Loss={val_losses[:,i].mean():.4f}, Grad Norm={total_norm:.4f}")
    ...
    print("[Decoder Training] Starting decoder training")
    ...
    for j in range(decoding_training_epochs):
        if j in decoding_learning_rate_reduction_factors.keys():
            for n in range(len(layer_numbers)):
                for param_group in decoding_optimizers[n].param_groups:
                    ...
                    print(f"[Decoder Training] Learning Rate changed to: {param_group['lr']}")
        for n, n_layer in enumerate(layer_numbers):
        ...
        if not j % train_freq_print and j:
            print(f"[Decoder Training] Epoch {j}: Train Loss={decoding_running_losses.mean():.4f}, Val Loss={val_decoding_losses[:,j].mean():.4f}, Grad Norm={total_norm:.4f}")
    ...
    print(f"[Model Save] Saved: {curr_dir}/models/encoders_state_dict_{run_name}.pth")
    ...
    print(f"[Model Save] Saved: {curr_dir}/models/decoders_state_dict_{run_name}.pth")

    print("[Testing] Running error statistics for testing data")
    ...
    with torch.no_grad():
        for n, layer in enumerate(layer_numbers):
            row_count = 0
            if verbose:
                print(f"[Testing] Layer {layer.item()}")
            e = 0
            ...
            print(f"[Testing] Layer {layer.item()}: Avg Error={errors[n]:.4f} digits, PT Error={problem_type_error}/{len(labels)}")
            print(f"[Testing] Layer {layer.item()}: Error per digit: {per_digit_errors[layer.item()]}")
    ...
    print(f"[Testing] Minimum error: {lowest_error:.4f} at layer {lowest_error_layer.item()}, PT error: {lowest_pt_error}")

    print("[Testing] Running error statistics for training data")
    ...
    with torch.no_grad():
        for n, layer in enumerate(layer_numbers):
            row_count = 0
            if verbose:
                print(f"[Training Eval] Layer {layer.item()}")
            e = 0
            for batch_idx, (data, labels) in enumerate(training_encoder_data_loaders[n]):
                row_count += len(data)
                pred = encoders[n](data)
            ...
            print(f"[Training Eval] Layer {layer.item()}: Avg Error={errors[n]:.4f} digits, PT Error={problem_type_error}/{len(labels)}")
            print(f"[Training Eval] Layer {layer.item()}: Error per digit: {per_digit_errors[layer.item()]}")
